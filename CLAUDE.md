...
- **Active configs** (current: R4/R5 width series, R7 GRAM):
  ...
  - GRAM stochastic latent transitions: `urm_r7a_gram_h128.yaml` (leaked — documented negative), `urm_r7b_gram_predecode_h128.yaml` (two-pass leak — documented negative), `urm_r7c_gram_bottleneck_h128.yaml` (KL collapsed — documented), `urm_r7c2_gram_nodetach_warmup_h128.yaml` (anti-refining — documented negative), `urm_r7d_gram_deltacal_h128.yaml` (delta-norm σ — documented negative, α sweep: `urm_r7d_a01_gram_deltacal_h128.yaml`, `urm_r7d_a03_gram_deltacal_h128.yaml`), `urm_r7e_gram_cleandecode_h128.yaml` (GRAM-faithful clean decode, fixes R7d structural bugs)
...
Ten model config fields control GRAM stochastic latent transitions (R7):
...
- **`gram_predecode`**: bool (default false). DEPRECATED in R7e — retained for backward-compat with old YAML configs but no longer read. R7e always decodes from clean u_t (the deterministic post-transformer state) before any perturbation.
...
- **`gram_sigma_alpha`**: float (default 0.0). R7d: when >0, overrides learned sigma magnitude. Eps is normalized to unit norm per example and rescaled to α·‖Δ_t‖ (the deterministic update's per-position L2 norm, mean over positions). Provides calibration and automatic annealing. Logs `gram_sigma_effective`.
