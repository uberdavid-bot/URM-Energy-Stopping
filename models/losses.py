...
class EnergyLossHead(nn.Module):
    ...
    def forward(
        ...
        **model_kwargs,
        ...
        # Per-step exact accuracy and delta norms (eval only, for convergence analysis)
        with torch.no_grad():
            if "per_step_logits" in outputs:
                for step_idx, step_logits in enumerate(outputs["per_step_logits"]):
                    ...
                    metrics[f"step_{step_idx + 1}_exact_accuracy"] = (valid_metrics & step_seq_correct).sum()
